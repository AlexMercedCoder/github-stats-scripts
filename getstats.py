...
def get_contributions_by_month(owner, repo, token):
    ...
    if response.status_code == 200:
        ...
        for week in data:
            ...
            # Check if this month already exists in df
            if month_end in df['date'].values:
                df.loc[df['date'] == month_end, 'total_contributions'] += week['total']
            else:
                df = df.append({'date': month_end, 'total_contributions': week['total']}, ignore_index=True)
    else:
        ...
        if response.status_code == 200:
            ...
            for pr in pull_requests:
                ...
                if merged_at:
                    ...
                    # Check if this month already exists in df
                    if month_end in df['date'].values:
                        df.loc[df['date'] == month_end, 'total_pull_requests'] += 1
                    else:
                        df = df.append({'date': month_end, 'total_pull_requests': 1}, ignore_index=True)
...
def get_contributors_by_month(owner, repo, token):
    ...
    if response.status_code == 200:
        ...
        for date, ids in contributors_count.items():
            df = df.append({'date': date, 'total_contributors': len(ids)}, ignore_index=True)
